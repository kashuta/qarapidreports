...
function ForkliftForm({ location }) {
  ...
  let formFields;
  let engineOffChecklist;
  let engineOnChecklist;

  if (reportsFields) {
    formFields = reportsFields.find((el) => el.formId === +formId);
    const questions = formFields?.questionFields.map((el) => {
      const obj = {};
      [obj.item, obj.hint] = el.question.split('; ');
      return obj;
    });
    engineOffChecklist = questions?.slice(0, 12);
    engineOnChecklist = questions?.slice(12);
